...
<div class="max-w-3xl space-y-8">
	...
	<section class="space-y-4">
		...
		<div class="space-y-2">
			<p><strong>Option 1: Use the startup script (Recommended)</strong></p>
			<CodeBlock
...
./scripts/start-codex.sh`}
			...
			<p class="text-xs text-muted-foreground">
				The script automatically handles macOS proxy issues and provides colored output.
			</p>
		</div>
		
		<div class="space-y-2">
			<p><strong>Option 2: Manual startup</strong></p>
			<CodeBlock
...
llm-link --app codex --provider aliyun --api-key <YOUR_ALIYUN_KEY> --model qwen3-coder-plus --auth-key your-token`}
		...
		<div class="space-y-3 mt-4">
			<h3 class="text-lg font-medium">Command Line Options</h3>
...
llm-link --app codex --provider zhipu --api-key <ZHIPU_KEY> --model glm-4.6`}
	...
	<section class="space-y-4">
		...
		<p class="text-sm text-muted-foreground">
			Update your <code>~/.codex/config.toml</code> to define LLM Link as a provider and create profiles for Zhipu GLM-4.6 (recommended) and Aliyun (alternative):
		</p>
		<CodeBlock
...
model_provider = "llm_link"`}
		...
		<div class="space-y-2 text-xs text-muted-foreground">
			<p class="font-semibold text-foreground text-sm">Authentication</p>
			<CodeBlock
...
llm-link --app codex --provider aliyun --model qwen3-coder-plus --api-key YOUR_ALIYUN_API_KEY --auth-key my-secret`}
			...
			/>
			<p>
				Codex will send the auth token as <code>Authorization: Bearer ...</code> when calling llm-link. If you start
				llm-link without <code>--auth-key</code>, you can omit <code>env_key</code> from the codex.toml configuration
				entirely (Codex will still be able to reach the proxy).
			</p>
		</div>
	</section>

	<section class="space-y-4">
		<h2 class="text-2xl font-semibold">3. Start Using Codex CLI</h2>
		<p class="text-sm text-muted-foreground">
			Once llm-link is running and Codex CLI is configured, you can start using Codex with your chosen LLM provider.
		</p>
		<CodeBlock
...
codex --help`}
			...
			/>
			<p>
				Make sure llm-link is running in the background (http://localhost:8088) before using Codex CLI. 
				You'll see responses from your configured LLM provider (Zhipu, Aliyun, etc.) through the proxy.
			</p>
		</div>
	</section>

	<section class="space-y-4">
		<h2 class="text-2xl font-semibold">4. Switching Providers</h2>
		<p class="text-sm text-muted-foreground">
...
llm-link --app codex --provider volcengine --api-key <VOLC_KEY> --model doubao-seed-1.6`}
	...
	<section class="space-y-4">
		<h2 class="text-2xl font-semibold">5. Troubleshooting</h2>
		
		<div class="space-y-3">
			<h3 class="text-lg font-medium">502 Bad Gateway Errors</h3>
			<p class="text-sm text-muted-foreground">
				If you encounter <code>502 Bad Gateway</code> errors when using Codex CLI, this is typically caused by proxy conflicts on macOS systems.
			</p>
			
			<div class="space-y-2">
				<p><strong>Symptoms:</strong></p>
				<ul class="list-disc list-inside text-sm space-y-1">
					<li>Codex CLI shows: <code>stream error: exceeded retry limit, last status: 502 Bad Gateway</code></li>
					<li>Direct curl requests to llm-link work fine</li>
					<li>llm-link logs show proxy usage: <code>proxy(http://127.0.0.1:7890) intercepts</code></li>
				</ul>
			</div>
...
./scripts/start-codex.sh`}
		...
		<div class="space-y-3">
			<h3 class="text-lg font-medium">Model Mismatch</h3>
			<p class="text-sm text-muted-foreground">
				Ensure the model in your <code>~/.codex/config.toml</code> matches the model llm-link is running with.
			</p>
...
model_provider = "llm_link"`}
		...
		<div class="space-y-3">
			<h3 class="text-lg font-medium">Authentication Issues</h3>
			<p class="text-sm text-muted-foreground">
				If you get <code>401 Unauthorized</code> errors, check that the authentication token matches between llm-link and Codex configuration.
			</p>
...
env_key = "LLM_LINK_AUTH_TOKEN"`}
		...
		<div class="space-y-3">
			<h3 class="text-lg font-medium">Tool Calls Not Working</h3>
			<p class="text-sm text-muted-foreground">
				If you notice Codex CLI showing tool calls as raw text (like <code>update_plan({'{...}'})</code>) instead of executing them, this is due to API compatibility issues with certain providers.
			</p>
			
			<div class="space-y-2">
				<p><strong>Symptoms:</strong></p>
				<ul class="list-disc list-inside text-sm space-y-1">
					<li>Tool calls appear as text instead of being executed</li>
					<li>Functions like <code>apply_patch</code>, <code>shell</code> show as raw text</li>
					<li>Model returns explanations instead of calling tools</li>
				</ul>
			</div>
			
			<div class="space-y-2">
				<p><strong>Affected Providers:</strong></p>
				<ul class="list-disc list-inside text-sm space-y-1">
					<li><strong>Aliyun</strong>: qwen3-coder-plus returns text, qwen-max errors with tool_choice</li>
					<li><strong>Other providers</strong>: May have varying OpenAI tools API compatibility</li>
				</ul>
			</div>
			...
			<div class="space-y-2">
				...
				<p class="text-sm text-muted-foreground">
					For tool-heavy workflows, use providers with confirmed OpenAI tools API compatibility:
				</p>
				<CodeBlock
...
# Note: Aliyun has confirmed tools API incompatibility issues`}
				...
				/>
				<p class="text-sm text-muted-foreground">
					For simple text generation without tools, Aliyun providers work well.
				</p>
			</div>
		</div>
		
		<div class="space-y-3">
			<h3 class="text-lg font-medium">Tool Calls Duplicating (Fixed)</h3>
			<p class="text-sm text-muted-foreground">
				Previous versions had streaming tool_calls duplication issues. This has been fixed in llm-connector 0.5.4+.
			</p>
			
			<div class="space-y-2">
				<p><strong>Fixed Issues:</strong></p>
				<ul class="list-disc list-inside text-sm space-y-1">
					<li>Streaming tool_calls delta accumulation now works properly</li>
					<li>No more repeated tool execution in streaming mode</li>
					<li>Compatible with providers that support OpenAI tools format</li>
				</ul>
			</div>
		</div>
	</section>
